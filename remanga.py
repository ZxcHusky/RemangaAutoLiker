...
class Remanga:
    ...
    def get_title_chapters(self, branch_id: int, page: int = 1, count: int = 1000) -> dict:
        """Получение глав с пагинацией"""
        return requests.get(
            f"{self.api}/api/titles/chapters/?branch_id={branch_id}&page={page}&count={count}",
            headers=self.headers).json()

    def get_all_chapters(self, branch_id: int) -> list:
        """Получение ВСЕХ глав через перебор страниц"""
        all_chapters = []
        page = 1

        while True:
            response = self.get_title_chapters(branch_id, page=page)
            
            # Проверка на ошибки
            if not response or "content" not in response:
                print(f"Ошибка API на странице {page}")
                break

            chapters = response["content"]
            if not chapters:
                print(f"Главы закончились (страница {page})")
                break

            all_chapters.extend(chapters)
            print(f"Страница {page}: получено {len(chapters)} глав")

            # Условие выхода: если глав меньше 100, это последняя страница
            if len(chapters) < 100:
                break
